...
/**
 * Backend: GPT-5 (no temperature) + DOC-FIRST + AUTO-RUN
 * - Никаких подтверждений: сохраняем скрипт и сразу выполняем его
 * - Инструменты: web.search (Bing/SerpAPI/GoogleCSE/Brave → fallback DuckDuckGo Lite),
 *   openapi.probe/load, http, script.save/run, env.list
 * - Маршруты: /api/health, /api/env, /api/env-file (GET/POST), /api/scripts (GET/POST)
 * - Безопасные HTTP: http/https только, запрет приватных IP
 */
...
/* ---------- SYSTEM PROMPT (plain string, no backticks) ---------- */
const SYSTEM_PROMPT =
'Ты — агент по внешним API. Действуй решительно.\\n' +
'1) Сначала web.search и openapi.probe/openapi.load, не выдумывай эндпоинты.\\n' +
'2) Если не хватает env/обязательных полей — спроси только их.\\n' +
'3) После подготовки запроса сохраняй скрипт (script.save). Сервер выполнит его автоматически.\\n'
;
